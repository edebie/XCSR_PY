...
class XCSRProgram:
    ...
    def run_experiments(self):
        for exp in range(conf.max_experiments):
            ...
            self.pop = XCSRClassifierSet(self.actual_time)
            self.init()
    ...
    def run_explor(self):
        ...
        gb_env.set_state()
        """MatchSet[M]を生成"""
        self.match_set = XCSRMatchSet(self.pop,self.actual_time)
        ...
        self.action_set = XCSRActionSet(self.match_set,self.action,self.actual_time)
        """行動を取る. 強化学習が働く"""
    ...
    def select_action(self):
        if random.random() > conf.p_explr:
            ...
            self.action = gb_env.random_action()
    ...
    def generate_prediction_array(self):
        self.p_array = [0] * GB_ACTION_TYPES_COUNT # Hold the prection value for each action
        self.f_array = [0] * GB_ACTION_TYPES_COUNT
    ...
    def apply_crossover(self,cl1,cl2):
        ...
        for i in range(sep1,sep2):
            if gb_env.equal_attribute_vals(cond1[i], cond2[i]):
    ...
    def apply_mutation(self,cl):
        ...
        for i in range(len(cl.condition)):
            if random.random() < conf.myu:
                if cl.condition[i] == '#':
                    cl.condition[i] = gb_env.state[i]
                else:
                    cl.condition[i] = '#'
        if random.random() < conf.myu:
            cl.action = gb_env.random_action()
