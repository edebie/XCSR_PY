...
class XCSRClassifier:
    ...
    def deep_copy(self,actual_time):
        cl = XCSRClassifier(self.condition,actual_time)
    ...
    def deletion_vote(self,ave_fitness):
        ...
        return vote
    def equals(self,cl):
        return gb_env.are_equal_conditions(self.condition, cl.condition) and
    ...
    def is_more_general(self,cl_spec):
        ...
        for i in range(len(self.condition)):
            is_equal = gb_env.equal_attribute_vals(self.condition[i], cl_spec.condition[i])
            if self.condition[i] != '#' and (is_equal == False):
                return False
            elif (is_equal == False):
                ret = True
        return ret
...
#     a = [XCSRClassifier([0,0,1],0),XCSRClassifier([0,1,0],1)]
#     for cl in a:
#         cl.action += 1
#         print cl.action
